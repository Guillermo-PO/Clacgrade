...
const App = {
  ...
  renderChart() {
    ...
    analyticsChartInstance = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          label: 'Promedio', data,
          borderColor: accentColor, backgroundColor: 'rgba(93, 123, 255, 0.1)',
          borderWidth: 2, fill: true, tension: 0.3, pointBackgroundColor: accentColor,
        }],
      },
      options: {
        responsive: true,
  ...
  resetAll() {
    ...
    State.materias = [];
    DB.save({ materias: [], minPass: State.minPass, historial: State.historial });
    showScreen('s-setup');
  },
  ...
  deleteMateria() {
    ...
    DB.save({ materias: State.materias, minPass: State.minPass, historial: State.historial });
  ...
  calcTargetGrade(val) {
    ...
    if (isNaN(target) || target < 0 || target > 10) {
      resEl.innerHTML = 'Ingresa una calificación objetivo válida (entre 0 y 10).';
      return;
    }
    ...
    if (maxPossible < pass) {
      return `<div class="predict-box">
        ...
        <p style="font-size:13px;color:var(--red);line-height:1.6">
          ...
          tus ítems pendientes, el máximo alcanzable es <strong>${fmt(maxPossible)}</strong>.
        </p>
      </div>`;
    }
  ...
  commitGrade(ri, gi, val, inputEl) {
    ...
    const predictWrap = $('predict-wrap');
    if (predictWrap) predictWrap.innerHTML = App._buildPredictHtml(mat);

    const targetInp = $('inp-target');
    if (targetInp && targetInp.value) App.calcTargetGrade(targetInp.value);
  },
