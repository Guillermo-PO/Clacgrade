...
function badgeForGrade(g) {
  if (g === null || isNaN(g)) return '<span class="badge badge-blue">Sin datos</span>';
  if (g >= State.minPass + 1.5) return '<span class="badge badge-green">✓ Aprobado</span>';
  if (g >= State.minPass)       return '<span class="badge badge-amber">⚠ En riesgo</span>';
...
const DB = {
  ...
  async save(payload) {
    ...
    const { error } = await _supa
      .from('profiles')
      .upsert({ id: State.user.id, data: payload, updated_at: new Date().toISOString() });
...
function scheduleSave() {
  ...
  State.syncTimer = setTimeout(async () => {
    await DB.save({ materias: State.materias, minPass: State.minPass });
  }, 1200);
}
...
const Auth = {
  ...
  async init() {
    ...
    _supa.auth.onAuthStateChange(async (event, session) => {
      if (event === 'SIGNED_IN' && session?.user) {
        ...
        State.user = null;
        showScreen('s-auth');
        // NO rotamos la frase en SIGNED_OUT — solo la mostramos al cargar la app
        // para que el fallo de OAuth no la cambie visualmente
        Auth.renderForm();
      }
    });
  },
  ...
  async submit() {
    ...
    const fn = Auth.currentTab === 'login'
  ...
  async loginGoogle() {
    ...
    // Usa siempre el origin limpio (sin path, sin hash, sin query params)
    // para que coincida exactamente con la Redirect URL registrada en Supabase.
    ...
    // Si no hay error, el navegador redirige solo — no hace falta hacer nada más aquí.
  },
  ...
  async logout() {
    ...
    State.materias = [];
    // Mostramos la pantalla de auth con la misma frase que ya tenía
    // (no la rotamos — eso solo ocurre en la primera carga)
    showScreen('s-auth');
    Auth.renderForm();
...
const Calc = {
  ...
  // Motor predictivo basado en ítems pendientes (r.pendientes)
  // Para cada rubro con ítems pendientes calcula el promedio exacto
  // necesario en esos ítems para alcanzar minPass en la materia total.
  // Fórmula: neededX = [ (pass - securedOthers) * (existingCount + pend) / wi - sumExistentes ] / pend
  predictPerRubro(mat) {
    ...
    return mat.rubros.map((r, ri) => {
      ...
      // Puntos asegurados por los OTROS rubros
      let securedOthers = 0;
...
const App = {
  ...
  // ── Reset ──────────────────────────────────────
  resetAll() {
    if (!confirm('¿Borrar toda la configuración y empezar de nuevo?')) return;
    State.materias = [];
    DB.save({ materias: [], minPass: State.minPass });
    showScreen('s-setup');
  },
  ...
  buildConfigForm() {
    ...
    for (let i = 0; i < total; i++) {
      pips += `<div class="step-pip ${i < idx ? 'done' : i === idx ? 'active' : ''}"></div>`;
    }
    ...
    const rows = mat.rubros.map((r, i) => `
      <div class="rubro-config-row">
        ...
        <input type="number" min="0" max="100" value="${r.porcentaje}" style="text-align:center"
    ...
    let nav = '';
    if (idx > 0) nav += `<button class="btn-ghost" onclick="App.cfgPrev()">← Anterior</button>`;
    nav += idx < total - 1
  ...
  cfgAddRubro() {
    State.materias[State.configIdx].rubros.push({ nombre: '', porcentaje: 0, calificaciones: [], pendientes: 0 });
    App.buildConfigForm();
  },
  cfgRemRubro(i) {
  ...
  cfgUpdateBar() {
    ...
    const fill  = $('cfg-pbar'), lbl = $('cfg-ptotal');
    if (!fill) return;
  ...
  cfgValidate() {
    ...
    const total = mat.rubros.reduce((s, r) => s + (parseFloat(r.porcentaje) || 0), 0);
    const err   = $('cfg-err');
  ...
  async cfgFinish() {
    ...
    State.materias.forEach(m => m.rubros.forEach(r => {
  ...
  buildDashboard() {
    ...
    $('dash-summary').innerHTML = `
      <div class="metric">
        ...
        <div class="big-grade ${gradeClass(globalAvg)}" style="font-size:36px">${globalAvg !== null ? fmt(globalAvg) : '—'}</div>
        <div class="m-sub">${avgs.length ? 'Promedio de materias con datos' : 'Sin calificaciones aún'}</div>
      </div>
      <div class="metric">
        <div class="m-label">Aprobadas</div>
        <div class="m-val" style="font-family:var(--mono);font-size:32px;font-weight:500;color:var(--green)">${avgs.length ? passing : '—'}</div>
    ...
    $('dash-cards').innerHTML = mats.map((m, i) => {
      ...
      const bars    = m.rubros.map((r, j) =>
        `<div class="drb" style="flex:${r.porcentaje};background:${Calc.rubroAvg(r) !== null ? colors[j % colors.length] : 'var(--bg3)'}"></div>`
      ).join('');
      ...
      return `<div class="card card-clickable" onclick="App.openMateria(${i})">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:12px">
          <div>
            <div style="font-size:16px;font-weight:500;margin-bottom:3px">${m.nombre || `Materia ${i + 1}`}</div>
            <div style="font-size:12px;color:var(--text3)">${m.rubros.length} rubros · ${covered}% cubierto${totalPend > 0 ? ` · ${totalPend} pendiente${totalPend > 1 ? 's' : ''}` : ''}</div>
          </div>
          <div style="text-align:right">
            <div class="big-grade ${gradeClass(avg)}" style="font-size:28px">${avg !== null ? fmt(avg) : '—'}</div>
            ${badgeForGrade(avg)}
          </div>
        </div>
  ...
  deleteMateria() {
    ...
    DB.save({ materias: State.materias, minPass: State.minPass });
    ...
    const rubroHtml = mat.rubros.map((r, ri) => {
      ...
      return `<div class="rubro-grade-row">
        ...
        <div class="pendientes-row">
          <span class="pendientes-label">📋 ¿Cuántos <strong>${r.nombre}</strong> faltan por calificar?</span>
    ...
    $('mat-detail').innerHTML = `
      ...
      <div class="metrics">
        <div class="metric">
          ...
          <div id="live-avg" class="big-grade ${gradeClass(avg)}">${avg !== null ? fmt(avg) : '—'}</div>
          <div class="m-sub">Solo rubros con datos</div>
        </div>
        <div class="metric">
          <div class="m-label">Calificación total</div>
          <div id="live-total" class="big-grade ${gradeClass(total)}">${fmt(total)}</div>
  ...
  setPendientes(ri, val) {
    State.materias[State.activeMateria].rubros[ri].pendientes = Math.max(0, val);
    scheduleSave();
    App.buildMateriaDetail();
  },
  ...
  _buildPredictHtml(mat) {
    ...
    // All rubros have grades, no pending items
    if (!hasPend) {
      ...
      if (allFilled) {
        return `<div class="predict-box" style="text-align:center">
          <p style="font-size:13px;color:var(--text2)">
            ${total >= pass
              ? `✓ Materia aprobada con promedio <strong style="color:var(--green)">${fmt(total)}</strong>`
              : `✗ Materia reprobada con promedio <strong style="color:var(--red)">${fmt(total)}</strong>`}
          </p>
        </div>`;
      }
      // Some rubros have no grades AND no pendientes set
      return `<div class="predict-box">
        <p class="predict-title">¿Qué necesitas para aprobar con ${pass.toFixed(1)}?</p>
    ...
    // Max possible if all pending items score 10
    ...
    const rows = perRub.map(({ rubro: r, pend, neededX }) => {
      ...
      return `<div class="predict-row">
        <span class="predict-rubro">
          ...
          <span style="color:var(--text3);font-size:11px"> (${r.porcentaje}%)</span><br>
          <span style="font-size:11px;color:var(--text3)">
            Promedio necesario en ${pend} ítem${pend > 1 ? 's' : ''} pendiente${pend > 1 ? 's' : ''}
          </span>
        </span>
  ...
  commitGrade(ri, gi, val) {
    ...
    let count = 0;
    for (let r = 0; r < State.materias[State.activeMateria].rubros.length; r++) {
      for (let g = 0; g < State.materias[State.activeMateria].rubros[r].calificaciones.length; g++) {
        if (r === ri && g === gi && inputs[count]) inputs[count].value = final === '' ? '' : final;
        count++;
      }
    }
    scheduleSave();
  ...
  updateGrade(ri, gi, val) {
    ...
    State.materias[State.activeMateria].rubros[ri].calificaciones[gi] =
      (raw === '' || raw === null) ? '' : isNaN(p) ? raw : Math.max(0, Math.min(10, p));
    scheduleSave();
  ...
  _updateAddBar() {
    ...
    const fill  = $('add-pbar'), lbl = $('add-ptotal');
    if (!fill) return;
    fill.style.width      = Math.min(100, total) + '%';
    fill.style.background = total === 100 ? 'var(--green)' : total > 100 ? 'var(--red)' : 'var(--amber)';
    lbl.textContent       = total.toFixed(1) + '%';
